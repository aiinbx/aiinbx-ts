...
```sh
export AI_INBX_API_KEY="My API Key"
npx -y aiinbx-mcp@latest
```
...
```json
{
  "mcpServers": {
    "aiinbx_api": {
      "command": "npx",
      "args": ["-y", "aiinbx-mcp", "--client=claude", "--tools=all"],
      "env": {
        "AI_INBX_API_KEY": "My API Key"
      }
    }
  }
}
...
[![Add to Cursor](https://cursor.com/deeplink/mcp-install-dark.svg)](https://cursor.com/en-US/install-mcp?name=aiinbx-mcp&config=eyJjb21tYW5kIjoibnB4IiwiYXJncyI6WyIteSIsImFpaW5ieC1tY3AiXSwiZW52Ijp7IkFJX0lOQlhfQVBJX0tFWSI6IlNldCB5b3VyIEFJX0lOQlhfQVBJX0tFWSBoZXJlLiJ9fQ)
...
[Open VS Code](https://vscode.stainless.com/mcp/%7B%22name%22%3A%22aiinbx-mcp%22%2C%22command%22%3A%22npx%22%2C%22args%22%3A%5B%22-y%22%2C%22aiinbx-mcp%22%5D%2C%22env%22%3A%7B%22AI_INBX_API_KEY%22%3A%22Set%20your%20AI_INBX_API_KEY%20here.%22%7D%7D)
...
| ------------------- | ------------------------ | --------------- |
| `x-ai-inbx-api-key` | `apiKey` | BearerAuth |
...
import { server, endpoints, init } from "aiinbx-mcp/server";

// import a specific tool
import retrieveThreads from "aiinbx-mcp/tools/threads/retrieve-threads";
...
init({ server: myServer, endpoints: [retrieveThreads, myCustomEndpoint] });
```
